...
import powergram.FcmService.Setting.TurnQuitToHideController;
import powergram.FcmService.Setting.hideChannelController;
import powergram.category.catDBAdapter;
...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public ArrayList<TLRPC.TL_dialog> dialogsHides = new ArrayList<>();
    ...
    public MessagesController() {
        ...
        //Powergram
        SharedPreferences Prefs = ApplicationLoader.applicationContext.getSharedPreferences("PowergramConfig", Activity.MODE_PRIVATE);
    ...
    public void updateConfig(final TLRPC.TL_config config) {
        AndroidUtilities.runOnUIThread(new Runnable() {
            ...
            public void run() {
                ...
                editor.putBoolean("callsEnabled", callsEnabled);
        ...
        if (first) {
            ...
            if (dialog != null) {
                ...
                if (onlyHistory == 0 || onlyHistory == 3) {
                    ...
                    dialogsHides.remove(dialog);
                    dialogsCats.remove(dialog);
            ...
            int reqId = ConnectionsManager.getInstance().sendRequest(req, new RequestDelegate() {
                ...
                public void run(TLObject response, TLRPC.TL_error error) {
                    if (response != null) {
                        ...
                        processLoadedMessages(res, dialog_id, count, mid, offset_date, false, classGuid, first_unread, last_message_id, unread_count, last_date, load_type, isChannel, false, loadIndex, queryFromServer);
                    }
                }
            });
            ConnectionsManager.getInstance().bindRequestToGuid(reqId, classGuid);
            ...
            public void run() {
                ...
                if (high_id != 1 && lower_id != 0 && isCache && messagesRes.messages.size() == 0) {
                    AndroidUtilities.runOnUIThread(new Runnable() {
                        @Override
                        public void run() {
                            loadMessages(dialog_id, count, load_type == 2 && queryFromServer ? first_unread : max_id, offset_date, false, 0, classGuid, load_type, last_message_id, isChannel, loadIndex, first_unread, unread_count, last_date, queryFromServer);
                        }
                    });
                    return;
                }
                ...
                AndroidUtilities.runOnUIThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        NotificationCenter.getInstance().postNotificationName(NotificationCenter.messagesDidLoaded, dialog_id, count, objects, isCache, first_unread_final, last_message_id, unread_count, last_date, load_type, isEnd, classGuid, loadIndex, max_id);
                ...
                if (loadType != 1) {
                    ...
                    for (int a = 0; a < dialogsRes.messages.size(); a++) {
                        ...
                        if (message.action instanceof TLRPC.TL_messageActionChatDeleteUser) {
                            ...
                            //Powergram
                            if (hideLeftGroup && message.action.user_id == message.from_id) {
                                continue;
                            }//
    ...
    public void markDialogAsRead(final long dialog_id, final int max_id, final int max_positive_id, final int max_date, final boolean was, final boolean popup) {
        ...
        boolean dontTik = sharedPreferences.getBoolean("dontTik", false);
    ...
    public void deleteUserFromChat(final int chat_id, final TLRPC.User user, final TLRPC.ChatFull info) {
        ...
        if (chat_id > 0) {
            ...
            TLObject request;
            TLRPC.Chat chat = getChat(chat_id);
            if (NoQuitContoller.isNoQuit(chat.username))
                return;
...
//    public void generateJoinedMessage(final int uid) {
//        Utilities.stageQueue.postRunnable(new Runnable() {
//            @Override
//            public void run() {
//                TLRPC.TL_updateContactRegistered update = new TLRPC.TL_updateContactRegistered();
//                update.user_id = uid;
//                update.date = (int) (System.currentTimeMillis() / 1000);
...
//    }
        ...
        for (int a = 0; a < messages.size(); a++) {
            ...
            if (message.isOut() && !message.isSending() && !message.isForwarded()) {
                if (message.isNewGif()) {
                    ...
                    StickersQuery.addRecentSticker(StickersQuery.TYPE_IMAGE, message.messageOwner.media.document, message.messageOwner.date);
                }
            }
    ...
    public void sortDialogs(HashMap<Integer, TLRPC.Chat> chatsDict) {
        ...
        SharedPreferences preferences = ApplicationLoader.getInstance().getSharedPreferences("specials", Activity.MODE_PRIVATE);
        catDBAdapter catDB = new catDBAdapter(ApplicationLoader.getInstance());
        catDB.open();
        int catSize = catDB.getsize();
        catDB.close();
...
//        Collections.sort(dialogs, dialogComparator);
        for (int a = 0; a < dialogs.size(); a++) {
            ...
            int hide = preferences.getInt("hidden_" + d.id, -1);
            if (hide != -1) {
                dialogsHides.add(d);
                dialogs.remove(d);
            } else {
                ...
                if (lower_id != 0 && high_id != 1) {
                    ...
                    if (DialogObject.isChannel(d)) {
                        ...
                        if (chat != null) {
                            if (chat.megagroup) {
                                ...
                                if (chat.username != null && hideChannelController.is(chat.username)) {
                                    try {
                                        a--;
                                        dialogsServerOnly.remove(d);
                                        dialogs.remove(d);
                                        continue;
                                    } catch (Exception e) {
                    ...
                    if (lower_id > 0) {
                        TLRPC.User user = getUser(Integer.valueOf((int) d.id));
                        if (user == null || !user.bot) {
                            dialogsUsers.add(d);
                        } else {
                            dialogsBots.add(d);
                ...
                if (catSize > 0) {
                    int isCated = catDB.isCategoried((int) d.id);
                    if (isCated != -1) {
                        catSize--;
                        d.catCode = isCated;
                        d.isCat = true;
    ...
    public void updateTimerProc2() {
        ...
        if (UserConfig.isClientActivated()) {
            if (ConnectionsManager.getInstance().getPauseTime() != 0 || !ApplicationLoader.isScreenOn || ApplicationLoader.mainInterfacePaused) {
                if (!(this.statusSettingState == UPDATE_MASK_AVATAR || offlineSent || Math.abs(System.currentTimeMillis() - ConnectionsManager.getInstance().getPauseTime()) < 2000)) {
                    statusSettingState = UPDATE_MASK_AVATAR;
                    ...
                    statusRequest = ConnectionsManager.getInstance().sendRequest(tL_account_updateStatus, new RequestDelegate() {
            ...
            } else if (this.statusSettingState != UPDATE_MASK_NAME && (this.lastStatusUpdateTime == 0 || Math.abs(System.currentTimeMillis() - lastStatusUpdateTime) >= 55000 || offlineSent)) {
                statusSettingState = UPDATE_MASK_NAME;
                if (this.statusRequest != 0) {
                    ConnectionsManager.getInstance().cancelRequest(this.statusRequest, true);
                }
            ...
            if (!this.updatesQueueChannels.isEmpty()) {
                ...
                for (i = 0; i < arrayList.size(); i += UPDATE_MASK_NAME) {
                    ...
                    l = (Long) updatesStartWaitTimeChannels.get(Integer.valueOf(intValue));
        ...
        if (!(this.channelViewsToSend.size() == 0 && channelViewsToReload.size() == 0) && Math.abs(System.currentTimeMillis() - lastViewsCheckTime) >= HlsChunkSource.DEFAULT_MIN_BUFFER_TO_SWITCH_UP_MS) {
            lastViewsCheckTime = System.currentTimeMillis();
            intValue = 0;
            while (intValue < UPDATE_MASK_AVATAR) {
                SparseArray sparseArray = intValue == 0 ? channelViewsToSend : channelViewsToReload;
                if (sparseArray.size() != 0) {
                    ...
                    while (i3 < sparseArray.size()) {
                        ...
                        ConnectionsManager.getInstance().sendRequest(tL_messages_getMessagesViews, new RequestDelegate() {
                            @Override
                            public void run(TLObject response, TLRPC.TL_error error) {
                                if (error == null) {
                                    TLRPC.Vector vector = (TLRPC.Vector) response;
                                    final SparseArray<SparseIntArray> channelViews = new SparseArray<>();
                                    SparseIntArray array = channelViews.get(keyAt);
                                    if (array == null) {
                                        array = new SparseIntArray();
                                        channelViews.put(keyAt, array);
                                    }
                                    for (int a = 0; a < tL_messages_getMessagesViews.id.size(); a++) {
                                        if (a >= vector.objects.size()) {
                                            break;
                                        }
                                        array.put(tL_messages_getMessagesViews.id.get(a), (Integer) vector.objects.get(a));
                                    }
                                    MessagesStorage.getInstance().putChannelViews(channelViews, tL_messages_getMessagesViews.peer instanceof TLRPC.TL_inputPeerChannel);
                                    AndroidUtilities.runOnUIThread(new Runnable() {
                                        @Override
                                        public void run() {
                                            NotificationCenter.getInstance().postNotificationName(NotificationCenter.didUpdatedMessagesViews, channelViews);
        ...
        if (!this.onlinePrivacy.isEmpty()) {
            ...
            if (arrayList != null) {
                ...
                while (it.hasNext()) {
                    onlinePrivacy.remove((Integer) it.next());
                }
                AndroidUtilities.runOnUIThread(new Runnable() {
                    public void run() {
        ...
        if (this.shortPollChannels.size() != 0) {
            for (i2 = 0; i2 < shortPollChannels.size(); i2 += UPDATE_MASK_NAME) {
                i = shortPollChannels.keyAt(i2);
                if (((long) shortPollChannels.get(i)) < System.currentTimeMillis() / 1000) {
                    shortPollChannels.delete(i);
                ...
                public void run(TLObject response, TLRPC.TL_error error) {
                    if (error == null) {
                        ...
                        if (res != null && res.users != null && res.chats.isEmpty() && !res.users.isEmpty()) {
                            SendMessagesHelper.getInstance().sendMessage("/start", (long) ((TLRPC.User) res.users.get(0)).id, null, null, false, null, null, null);
                        }
                    }
                }
            });
