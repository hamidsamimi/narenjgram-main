...
import powergram.shamsi.ShamsiCalendar;
import powergram.shamsi.ShamsiDate;
...
//        systemDefaultLocale = Locale.getDefault();
        ...
        try {
            ...
            String lang = preferences.getString("language", "fa");
            if (lang != null) {
...
//    }
    ...
    public static String formatDateOnline(long j) {
        ...
        if (language.toLowerCase().equals("fa") || language.toLowerCase().equals("ku") ) {
            try {
                Calendar instance = Calendar.getInstance();
                i = instance.get(Calendar.DAY_OF_YEAR);
                i2 = instance.get(Calendar.YEAR);
                instance.setTimeInMillis(j * 1000);
                int i4 = instance.get(Calendar.DAY_OF_YEAR);
                i3 = instance.get(Calendar.YEAR);
                ShamsiDate dateToShamsi = ShamsiCalendar.dateToShamsi(new Date(j * 1000));
                if (i4 == i && i2 == i3) {
                    return String.format("%s %s %s", new Object[]{getString("LastSeen", R.string.LastSeen), getString("TodayAt", R.string.TodayAt), getInstance().formatterDay.format(new Date(j * 1000))});
                } else if (i4 + QUANTITY_ZERO == i && i2 == i3) {
                    return String.format("%s %s %s", new Object[]{getString("LastSeen", R.string.LastSeen), getString("YesterdayAt", R.string.YesterdayAt), getInstance().formatterDay.format(new Date(j * 1000))});
                } else if (i2 == i3) {
                    StringBuilder append = new StringBuilder().append(dateToShamsi.getDay()).append(" ");
                    language = language.toLowerCase().equals("fa") ? ShamsiCalendar.getShamsiMonth(dateToShamsi.getMonth()) : language.toLowerCase().equals("ku") ? ShamsiCalendar.getShamsiMonthKur(dateToShamsi.getMonth()) : ShamsiCalendar.getShamsiMonthEn(dateToShamsi.getMonth());
                    ...
                    language = formatString("formatDateAtTime", R.string.formatDateAtTime, objArr2);
                    objArr = new Object[QUANTITY_ONE];
                    objArr[QUANTITY_OTHER] = getString("LastSeenDate", R.string.LastSeenDate);
                    ...
                    language = formatString("formatDateAtTime", R.string.formatDateAtTime, objArr);
                    objArr = new Object[QUANTITY_ONE];
                    objArr[QUANTITY_OTHER] = getString("LastSeenDate", R.string.LastSeenDate);
                    objArr[QUANTITY_ZERO] = language;
                    return String.format("%s %s", objArr);
        ...
        try {
            ...
            i3 = instance2.get(Calendar.DAY_OF_YEAR);
            i = instance2.get(Calendar.YEAR);
            instance2.setTimeInMillis(j * 1000);
            i2 = instance2.get(Calendar.DAY_OF_YEAR);
            int i5 = instance2.get(Calendar.YEAR);
            if (i2 == i3 && i == i5) {
                return String.format("%s %s %s", new Object[]{getString("LastSeen", R.string.LastSeen), getString("TodayAt", R.string.TodayAt), getInstance().formatterDay.format(new Date(j * 1000))});
    ...
    public void recreateFormatters() {
        ...
        isRTL = (lang.toLowerCase().equals("ar") || lang.equalsIgnoreCase("fa") || lang.equalsIgnoreCase("ku") || lang.equalsIgnoreCase("fa_Fa"));
...
//            if (year != dateYear) {
//                if (isRTL){
//                    ShamsiDate shamsiDate = ShamsiCalendar.dateToShamsi(new Date(date * 1000));
//
//                    return shamsiDate.toDateString();
//                }else {
//                    return getInstance().formatterYear.format(new Date(date * 1000));
//                }
//            } else {
...
//    }
    ...
    public static String stringForMessageListDate(long date) {
        try {
            ...
            String language = locale.getLanguage();
            if (isRTL || language.toLowerCase().equals("fa") || language.toLowerCase().equals("ku")) {
                ShamsiDate dateToShamsi = ShamsiCalendar.dateToShamsi(new Date(date * 1000));
                if (year != dateYear) {
                    return dateToShamsi.toDateString();
                }
                day = dateDay - day;
                if (day == 0 || (day == -1 && ((long) ((int) (System.currentTimeMillis() / 1000))) - date < 28800)) {
                    return getInstance().formatterDay.format(new Date(date * 1000));
                }
                if (day <= -7 || day > -1) {
                    StringBuilder append = new StringBuilder().append(dateToShamsi.getDay()).append(" ");
                    ...
                    rightNow.setTimeInMillis(date * 1000);
                    return (String) ShamsiCalendar.getShamsiWeekDaysKyMap().get(Integer.valueOf(rightNow.get(Calendar.DAY_OF_WEEK)));
                }
            } else if (year != dateYear) {
                return getInstance().formatterYear.format(new Date(date * 1000));
            } else {
                int i5 = dateDay - day;
                return (i5 == 0 || (i5 == -1 && ((long) ((int) (System.currentTimeMillis() / 1000))) - date < 28800)) ? getInstance().formatterDay.format(new Date(date * 1000)) : (i5 <= -7 || i5 > -1) ? getInstance().formatterMonth.format(new Date(date * 1000)) : getInstance().formatterWeek.format(new Date(date * 1000));
            }
        } catch (Exception e) {
            FileLog.e( e);
        }
    ...
    public static String formatShortNumber(int number, int[] rounded) {
        String K = "";
        //Powergram
        if (true) return String.format(Locale.US, "%d%s", number, K);
        int lastDec = 0;
        int KCount = 0;
        while (number / 1000 > 0) {
            K += "K";
