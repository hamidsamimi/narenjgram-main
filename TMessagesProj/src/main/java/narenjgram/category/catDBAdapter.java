...
public class catDBAdapter {
    ...
    private static final String DATABASE_NAME = "Powergram";
    ...
    private static final String TAG = "Powergram_USER_TABLE_CATS";
    ...
    public chatobject getItm(int ID) throws SQLException {
        Cursor cursor = this.db.query(true, DATABASE_MAINTABLE, this.yek_SH_flashkart, "dialog_id == '" + ID + "'", null, null, null, null, null);
        chatobject nam = new chatobject();
  ...
  public boolean isExist (int did ){
        Cursor cursor = db.rawQuery("SELECT COUNT(*) FROM catchats Where dialog_id == '" + did + "'", null);
      cursor.moveToFirst();
        boolean isOk = cursor.getInt(0) > 0 ;
        cursor.close();
        return isOk;
