...
public class CustomDialogAdapter extends RecyclerView.Adapter {
    ...
    public CustomDialogAdapter(Context context, int type) {
        mContext = context;
        dialogsType = type;
    }
    ...
    public void onViewAttachedToWindow(RecyclerView.ViewHolder holder) {
        if (holder.itemView instanceof CustomDialogCell) {
            ((CustomDialogCell) holder.itemView).checkCurrentDialogIndex();
        }
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        View view = null;
        if (viewType == 0) {
            view = new CustomDialogCell(mContext);
        } else if (viewType == 1) {
            view = new LoadingCell(mContext);
        }
        view.setLayoutParams(new RecyclerView.LayoutParams(155, RecyclerView.LayoutParams.MATCH_PARENT));
